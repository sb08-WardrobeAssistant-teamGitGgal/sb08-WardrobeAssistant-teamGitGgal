...
class FeedSearchCacheServiceTest {
  ...
  class CacheHit {
    ...
    void searchByContent_CacheHit_ReturnsCachedResult() {
      ...
      given(objectMapper.convertValue(any(), any(TypeReference.class)))
  ...
  class CacheMiss {
    ...
    void searchByContent_CacheMiss_QueriesESAndSavesToCache() {
      ...
      then(valueOps).should().set(eq(CACHE_KEY), eq(List.of(mockDoc)), eq(Duration.ofMinutes(3)));
    }
  }
  ...
  class RedisFailure {
    ...
    void searchByContent_RedisSetFails_StillReturnsResult() {
      // given
      given(valueOps.get(CACHE_KEY)).willReturn(null);
      given(feedSearchRepository.searchByContent(KEYWORD)).willReturn(
          List.of(mock(FeedDocument.class)));
